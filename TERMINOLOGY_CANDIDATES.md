...
## Cross-datacenter replication API correction (added 2026-09-07)

Terms and phrasings introduced while validating
`docs/en/functions/95-disaster-recovery/` against a live business cluster
(operator `v5.1.0-rc.43.g2e4ef16f`, an ancestor of `release-5.1` whose only
intervening commits are e2e tests) and against `release-5.1` source. Repository
paths are relative to the sibling checkout `redis-group/redis-operator`.

| Term as used in docs | Provenance | Note for the reviewer |
|:---|:---|:---|
| **module peer port** (`7379`) | `internal/config/keys.go:54-57` — `ActiveRedisPeerPort = 7379`, "the module-owned peer listener port"; the Service port is appended only for mesh in `internal/builder/activeredis/service.go:66-80` | Replaces the docs' former "dedicated peer port", which implied a separate transport that Disaster Recovery links use. They do not: the port is a Service-level **alias onto the same proxy listener**, added only in Active-Active mode. Confirm the phrase, and confirm that "alias" is the right word for customer docs. |
| **default-account `RedisUser`** | `internal/builder/activeredis/peerauth.go:26-48` (`DefaultAccountRedisUserRef`); names generated by `internal/builder/clusterbuilder/acl.go:74-76` (`drc-acl-<name>-default`) and `internal/builder/failoverbuilder/acl.go:74-76` (`rfr-acl-<name>-default`) | **Coined for these docs.** Names the `RedisUser` object holding an instance's `default` account. As of `207d1c3c` (*accept the default account as a peer-auth credential*) this is the binding both setup guides now document for Redis `7.2`, and the one the operator writes for Redis `6.0`. The code has no customer-facing name for it. Confirm the phrase, and confirm that spelling the generated resource names out in the docs is wanted. |
| **peer-auth binding** | `api/middleware/redis/v1alpha1/activeredis_types.go:63-84` (`RedisUserRef`); `internal/controller/middleware/redis_peerauth_provision.go` | The docs already used *peer-auth credential* for the value. *Binding* is now used for the `spec.activeRedis.redisUserRef` reference itself, so that "the credential" and "the field naming it" stop being the same word. Confirm both are wanted. |
| **one endpoint carries both transports** | `internal/builder/activeredis/service.go:49-56` — "ONE proxy endpoint carries both transports. The proxy has a single listener and picks the backend port per session" | **Coined for these docs.** Describes why a Disaster Recovery link needs only the proxy RESP address. Confirm the phrasing; the alternative is to describe the per-session routing explicitly, which is more accurate but longer. |

Resolved by this change, and recorded here because the wrong names were
published: `spec.activeRedis.redisUserName` does not exist (the field is
`redisUserRef`, `api/middleware/redis/v1alpha1/activeredis_types.go:84`), and
`ActiveRedisConnection.spec.secretName` has been removed
(`internal/controller/middleware/activeredisconnection_controller.go:205-217`).
Both were verified against the live API by server-side dry-run. `aapeerrepl` is
gone from the product as well — `77851bdc` removed the mint along with
`secretName`, its only password source — so the docs no longer name it; it
survives only as a test constant in `tests/framework/activeredis/deployer.go:53`.

Checked and left unchanged: `status.upstreamPeer.service_metadata` is real —
`internal/controller/middleware/redis/activeredis_args.go:47-50` injects
`instance=<namespace>/<name>` on both generations, plus `version=` on the new
module. The live connection shows it empty only because
`activeredisconnection_controller.go:433-439` records `upstreamPeer` once, from
the first shard to report peer info, and that shard reported none.
