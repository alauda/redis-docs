---
weight: 10
sourceSHA: 7dbad43d5b14803a9dfc467f53502b2b669af44c36d98ad2bc8a7e6a33678193
---

# 如何访问哨兵实例

示例展示了如何使用 [go-redis](https://github.com/redis/go-redis)、[Jedis](https://github.com/redis/jedis)、[Lettuce](https://github.com/lettuce-io/lettuce-core) 和 [Redisson](https://github.com/redisson/redisson) 客户端访问 Redis（哨兵模式）。更多可选客户端，请 [查看 Redis 社区客户端推荐](https://redis.io/clients)。

## 获取访问信息

* 如果您已为 Redis 设置密码，需使用该密码。

* 如果您已禁用 **设置密码**，则可无需密码访问实例。<br/>

<Directive type="warning" title="注意">
**提示**：为确保环境安全，建议您参考 [用户管理](../../functions/20-user) 设置密码，并使用已配置的密码访问 Redis。
</Directive>
...
若创建实例时已启用 **集群外访问** 方法，客户端可通过实例 **访问方式** 页签中的 **集群外访问** 区域内的地址连接 Redis。

| 参数                       | 说明                                                                                                       |
| -------------------------- | ---------------------------------------------------------------------------------------------------------- |
| **哨兵节点访问地址**      | 用于从集群外访问实例，包括对应的哨兵服务的 IP 地址和端口。                                           |

## 调试 Redis

在实例详细信息页，单击右上角的 **终端控制台**，并通过 `redis-cli` 命令连接每个 Redis 节点。

```
redis-cli -h <对应端口 6379 的 IP> -p 6379
```
