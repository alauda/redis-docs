...
<Directive type="danger" title="Upgrade Redis 8.4 instances before upgrading to v5.1.0">
Alauda Cache Service for Redis OSS v5.1.0 ships no Redis `8.4` server image. A Redis `8.4` instance that survives the upgrade keeps serving traffic but can no longer be managed by the operator.

Upgrade every Redis `8.4` instance to Redis `7.2` **before** upgrading the operator to v5.1.0. See [Upgrade](../upgrade.mdx).
</Directive>

<Directive type="note" title="Minor version upgrades of active-active instances">
This page covers patch version upgrades. Moving an instance with active-active replication enabled between Redis minor versions has additional requirements — see [Upgrade a Peer-of Group from Redis 6.0 to Redis 7.2](../functions/95-disaster-recovery/50-upgrade-6.0-to-7.2.mdx).
</Directive>
