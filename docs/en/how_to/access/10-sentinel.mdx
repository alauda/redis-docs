...
# How to Access Sentinel Instances

This example demonstrates how to access Redis (in Sentinel mode) using the [go-redis](https://github.com/redis/go-redis), [Jedis](https://github.com/redis/jedis), [Lettuce](https://github.com/lettuce-io/lettuce-core), and [Redisson](https://github.com/redisson/redisson) clients. For more optional clients, [view the Redis community client recommendations](https://redis.io/clients).

## Obtaining Access Information

- If you have set a password for Redis, you need to use that password.

- If you have disabled **Set Password**, you can access the instance without a password.<br />

<Directive type="warning" title="Note">
  **Tip**: To ensure environment security, it is recommended that you refer to [User Management](../../functions/20-user) to set a password and use the configured password to access Redis.
</Directive>
...
Once the instance is created, you can find the information used for **internal cluster access** in the **Access Method** tab under the **Internal Cluster Access** section.

| Parameter                   | Description                           |
|-----------------------------|---------------------------------------|
| **Connection Address (by Internal Routing Name)** | Sentinel internal routing name and port. |
| **Connection Address (by Internal Routing IP)**   | IP address and port of the internal routing. |

### For External Cluster Access

If **External Cluster Access** was enabled when creating the instance, clients can connect to Redis through the node access addresses via the **External Cluster Access** section in the instance **Access Method** tab.

| Parameter                  | Description                                               |
|----------------------------|-----------------------------------------------------------|
| **Sentinel Node Access Address** | Used for accessing the instance from outside the cluster, including the IP addresses and port numbers of the corresponding internal routes of the sentinels. |
...
```
redis-cli -h {IP corresponding to port 6379} -p 6379
```
...
The following lists common example codes for connecting to Redis for reference.

> **Note**: The master-slave cluster name registered in Sentinel mode is fixed as `mymaster`.
