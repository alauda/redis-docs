...
The Alauda Cache Service E1 is engineered to provide service continuity and data security across all supported versions. This guide details version compatibility and outlines the recommended upgrade paths to facilitate a seamless transition for your environment.
...
The Alauda Cache Service E1 adheres to semantic versioning principles, providing the following compatibility guarantees:
...
| Alauda Cache Service E1 Version | Redis Server Versions  | ACP Version      |
|:--------------------------------|:-----------------------|:-----------------|
...
<Directive type="note" title="ACP Compatibility for v5.1.x">
Alauda Cache Service E1 v5.1.x runs on ACP v4.2, v4.3, and v4.4 only. It is not supported on ACP v4.1: upgrade ACP to v4.2 or later before upgrading Alauda Cache Service E1, or remain on v5.0.x.

Conversely, Alauda Cache Service E1 v5.0.x and earlier have not been tested on ACP v4.4. Before upgrading to ACP v4.4, please upgrade Alauda Cache Service E1 to v5.1.0 or later first.
</Directive>

<Directive type="note" title="ACP v4.3 Compatibility">
Alauda Cache Service E1 v4.1.x and v4.0.x have not been tested on ACP v4.3. Before upgrading to ACP v4.3, please upgrade Alauda Cache Service E1 to v5.0.0 or later first.
</Directive>

<Directive type="warning" title="Upgrade Redis 5.0 instances before upgrading to ACP v4.3">
Upgrading the Alauda Cache Service E1 operator does **not** automatically upgrade the Redis version of existing instances; each instance's Redis version must be upgraded explicitly.

Because ACP v4.3 requires Alauda Cache Service E1 v5.0.0 or later, and those versions no longer support Redis `5.0`, you must first upgrade every Redis `5.0` instance to a supported version (`6.0` or `7.2`) **before** upgrading from an ACP version earlier than v4.3 to v4.3.
...
<Directive type="note" title="Upgrading a replicating instance across Redis minor versions">
If an instance has cross-datacenter replication enabled, its Redis version cannot be changed in isolation. Follow [Upgrade a Disaster Recovery Group from Redis 6.0 to Redis 7.2](./functions/95-disaster-recovery/20-disaster-recovery/40-upgrade-6.0-to-7.2.mdx), which coordinates the whole replication group.
</Directive>

<Directive type="note" title="Replicating instances during the upgrade to v5.1">
Upgrading the operator to v5.1 does not interrupt an existing Disaster Recovery group. Replication continues across the upgrade and the data on both sides is unaffected — the instances stay on their current Redis version, so they keep running the module generation they were already using.

One thing does move. In v5.1 the link credential is declared on the instance through `spec.activeRedis.redisUserRef` instead of on the connection through `spec.secretName`, and the operator writes that binding for each replicating instance on its first reconcile after the upgrade. It names the instance's own default account — the account a Redis `6.0` link has always authenticated as — so nothing changes on the wire and no action is required.

Your manifests do need a change: `spec.secretName` is removed from `ActiveRedisConnection` and `ActiveRedisInspection`, and a manifest that still sets it is rejected on its next apply. See [Set Up Disaster Recovery Replication](./functions/95-disaster-recovery/20-disaster-recovery/20-setup.mdx#redis-60--legacy-module).
</Directive>
...
1.  Download the target version of the `Alauda Cache Service E1` plugin from AlaudaCloud to a node that has access to your ACP cluster.
...
3.  Upgrades follow the strategy configured in the Subscription:
    ...
    *   `Manual Upgrade`:
         <Tabs>
           ...
           - Navigate to **Marketplace** > **OperatorHub** > **Alauda Cache Service E1 detail page**.
