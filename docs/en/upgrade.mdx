...
Because ACP v4.3 requires Alauda Cache Service for Redis OSS v5.0.x, and v5.0.x no longer supports Redis `5.0`, you must first upgrade every Redis `5.0` instance to a supported version (`6.0` or `7.2`) **before** upgrading from an ACP version earlier than v4.3 to v4.3.
...
<Directive type="danger" title="Migrate Redis 8.4 instances before upgrading to v5.1.0">
Alauda Cache Service for Redis OSS v5.1.0 ships no Redis `8.4` server image, because the Redis 8.x license terms do not permit redistribution.

The operator resolves an instance's server image from the shipped image set on every reconciliation. After the upgrade, a surviving Redis `8.4` instance keeps serving traffic from its running pods, but **fails to reconcile** — the operator can no longer manage it, so configuration changes, scaling, failover handling, and upgrades stop being applied.

Before upgrading to v5.1.0, upgrade every Redis `8.4` instance to Redis `7.2`, and confirm that no instance reports `8.4` in the `VERSION` column:

```bash
$ kubectl get redis -A
```

Refer to the [Instance Upgrade Guide](./how_to/30-upgrade.mdx) for the upgrade procedure. If an instance has active-active replication enabled, follow [Upgrade a Peer-of Group from Redis 6.0 to Redis 7.2](./functions/95-disaster-recovery/50-upgrade-6.0-to-7.2.mdx) instead — the same constraints on group-wide coordination apply.
</Directive>
