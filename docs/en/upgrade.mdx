...
<Directive type="note" title="Upgrading an active-active instance across Redis minor versions">
If an instance has active-active replication enabled, its Redis version cannot be changed in isolation. Follow [Upgrade a Peer-of Group from Redis 6.0 to Redis 7.2](./functions/95-disaster-recovery/50-upgrade-6.0-to-7.2.mdx), which coordinates the whole replication group.
</Directive>
