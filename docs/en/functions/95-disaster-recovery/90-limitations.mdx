...
### The provided link address of the source instance needs to be highly available by itself

The Proxy address in the disaster recovery instance does not guarantee high availability. If you use the NodePort address in the target instance, you need a front-end load balancer to ensure high availability; you can also use MetalLB to achieve high-availability access to the Proxy.
...
1. The target side of the cluster mode can only be the cluster mode
1. The shards of the target side in cluster mode must be consistent with the source
1. The slot distribution of the target side in cluster mode must be consistent with the source
