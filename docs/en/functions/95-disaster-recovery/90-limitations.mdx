...
Run a time-synchronization daemon (`ntpd`, `chronyd`, or the platform equivalent) on every node of every cluster in an Active-Active group, and monitor the alarms. See [Clock synchronization](./30-active-active/30-operations.mdx#clock-synchronization-requirement) for the measurement, the two alarm tiers, and how to confirm that skew caused a particular problem.
...
In Active-Active mode on Redis `7.2` the address must expose **every port the proxy Service publishes**, each under its own number. The mesh reaches members on a port the module fixes itself and advertises unchanged, so remapping it breaks gossip. A Disaster Recovery link uses the RESP endpoint alone.
