...
* In **peer-of** mode, one upstream can have at most 15 downstream instances. Cascading is not supported — only star distribution, not tree or ring links.
* In **mesh** mode, a mesh can hold at most 16 members.

### Mesh mode is available on Redis 7.2 only

Mesh mode is rejected by admission on Redis `6.0`. Peer-of is available on both versions.
...
In mesh mode on Redis `7.2`, a DNS name is the recommended form for the announce address and for seed entries, because the module re-resolves it on every dial.
