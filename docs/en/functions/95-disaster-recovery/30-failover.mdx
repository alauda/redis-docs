...
After the failed source returns to normal, it can be re-added to the disaster recovery cluster as a target instance of the new source. For the operation method, refer to [Setup Disaster Recovery](./20-setup.mdx).


## Client-side Disaster Recovery Switch

Before connecting to the disaster recovery cluster, the client's access needs to support switching the access address of the Redis instance to switch to the new target instance after perceiving the source failure. Usually, there are the following switching methods:
