...
After the connection between the standby and the source becomes abnormal, the Disaster Recovery Tab of the Redis instance will prompt **Transmission status: Abnormal**, and a **Failover** button will appear in the upper left corner of the topology. When the **Failover** button appears, you can perform a disaster recovery cluster switch.
...
1. In the `Disaster Recovery` tab, click **Failover** button.
1. In the popup widge, click **Failover** to make sure do failover.
