...
i18n:
  title:
    zh: 配置容灾
title: Set Up Peer-of Replication
---

# Set Up Peer-of Replication

Peer-of (`spec.activeRedis.mode: peerof`) is the hot-standby / disaster recovery topology: an upstream instance fans out directed replication links to one or more downstream instances. Setting it up involves two parts — enabling active-active replication on each Redis instance, and establishing a connection from the downstream instance to the upstream instance.

<Directive type="info" title="Cluster Role Description">
In a peer-of group, the role of each Redis instance is not fixed; it can be either an upstream or a downstream. The downstream does not restrict data writing, but data written to the downstream will be overwritten by data synchronized from the upstream or cause synchronization failure due to data type conflicts.
</Directive>
...
Redis `6.0` carries the frozen legacy module, kept for compatibility with existing instances. It supports peer-of only — no mesh mode, and no module-level peer authentication. Creating an active-active instance on Redis `6.0` returns an admission warning recommending Redis `7.2`. For new deployments, use Redis `7.2`.
</Directive>
