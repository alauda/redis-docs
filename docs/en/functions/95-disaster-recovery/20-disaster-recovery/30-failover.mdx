...
**Active-Active** mode has no failover step of this kind: every member is already active and accepts writes, so recovering from the loss of a datacenter is a matter of redirecting clients to a surviving member. Removing the lost member from the mesh is described in [Removing a member](../30-active-active/30-operations.mdx#removing-a-member).
</Directive>
...
## Client-side Disaster Recovery Switch \{#client-side-disaster-recovery-switch}
