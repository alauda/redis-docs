...
5. **The upstream proxy endpoint stays reachable.** The new module replicates over the same proxy endpoint the Redis `6.0` links already used, so a Disaster Recovery group needs no new port opened. The proxy Service is not recreated by the upgrade — it keeps its address, its port and, under NodePort, its node port — so leave the existing firewall rules and load-balancer entries for the proxy's RESP port in place, and the links can be re-created at the end against the same addresses you recorded in step 6.
...
<Directive type="info" title="A link wired mid-upgrade parks rather than breaks">
The same guard covers this upgrade. A connection created while the upstream is already on Redis `7.2` and the downstream is still on `6.0` is accepted, then held — it does not replicate, and it does not damage either side:

```bash
$ kubectl -n default get activeredisconnections
NAME              INSTANCE   STATUS   MESSAGE                                                                                                  AGE
conn-dc2-to-dc1   s6-dc2     Paused   upstream version 7.2 newer than local version 6.0; waiting for local upgrade (safe order upgrades the downstream first)   18s
```

It stays `Paused` until the local member is upgraded. Following the procedure — links deleted in Step 2, re-created in Step 5 — you will not see this; it is what a mistimed link looks like if you do.
</Directive>
...
* `status.shards[].syncStatus` is `PartialSync` — the new module picks the link up from the offset the Redis `6.0` link left off at, so a re-created link resumes rather than re-sending the dataset. A `FullSync` here means the retained state was not usable and the whole dataset is being re-sent; it will still converge, but it costs a full transfer;
* the upstream's `ActiveRedis` reports the expected `status.downstreamPeerCount`;
* a key written on the upstream appears on each downstream, alongside the keys that were there before the upgrade.
...
* Downgrading from Redis `7.2` back to Redis `6.0` is not supported — and **nothing prevents it**. Setting `spec.version` back to `6.0` is accepted by admission and starts rolling the pods onto the older image immediately. If the upgrade must be abandoned, restore the members from the backups taken in the prerequisites rather than reversing the version.
