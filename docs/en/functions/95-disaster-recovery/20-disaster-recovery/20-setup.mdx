...
`status.shards[].versionState` is the module's own verdict on the upstream's version. `verified` means the two ends agree; a link held for version skew reports it here.
