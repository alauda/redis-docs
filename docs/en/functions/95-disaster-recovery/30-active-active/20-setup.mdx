...
<Directive type="warning" title="Enabling the mode restarts the data pods">
...
<Directive type="warning" title="Wait for the instance to be Ready first">
Step 2 rolls the pods, and the module arrives with them. Create the `ActiveRedisMesh` before the roll reaches the master and the mesh lands in `Failed`, reporting that the module's own commands do not exist:
...
This clears by itself — the mesh moves to `Pending` once the master carries the module, then to `Healthy` — but it is indistinguishable from a real failure while it lasts. Wait for `kubectl get redis` to report `Ready`.
</Directive>
...
<Directive type="info" title="The local member's entry is shorter than the others">
