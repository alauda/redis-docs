...
* Each member's proxy endpoint is reachable from every other member on **both** its announced RESP port (`6379` by default) and the peer port (`7379`), and the peer port is not remapped.
...
Neither gossip nor replication travels on `announcePort`. Both ride the **peer port**, and a member advertises its own local peer port (`7379`) for them — there is no announce override for that number.

The proxy runs a **single listener**: it carries application traffic, gossip, and replication on one port and tells them apart per session. The `7379` entry on the proxy Service is an alias onto that same listener, added in Active-Active mode only, not a second listener. An external load balancer or firewall in front of the proxy must therefore forward **both** numbers — `announcePort` and `7379` — to it, and must not remap `7379`.
</Directive>
...
Write seeds as plain `host:port` and let the operator complete them. It normalizes the list into the module's seed grammar before injecting it, appending the peer port — `redis-dc2.example.com:6379` becomes `redis-dc2.example.com:6379@7379`, which is the form `CONFIG GET activeredis.mesh-seeds` reports back. That completion is not cosmetic: a seed carrying no peer port is undialable, because gossip has no other transport. The peer port is fixed at `7379` and must not be remapped, so there is nothing here for you to override.
