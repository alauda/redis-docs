...
* **Membership is state that converges**, so it is reported rather than declared. The `ActiveRedisMesh` status carries the member list and each member's state. It also carries an `epoch`, which is not a change counter: it advances only when a member is forgotten or permanently decommissioned, so a group that has only ever gained members reports `0`.
...
The traffic itself — both the gossip rounds and the replicated Oplog — is carried on the **peer port**: a second number the proxy Service publishes onto that same listener, which you can read off the Service. The module fixes that number itself and advertises it unchanged, which is why an external load balancer or firewall in front of the proxy must publish it under the same number. Only the RESP port may be remapped.
