...
For the aggregated gauges and the attribution flag, read them on the instance itself. Run this **against the shard's primary node**: only the primary takes part in gossip, so the mesh counters on a replica describe whatever reached it passively and are not a view of the mesh.
...
Per-member offsets are also available directly from the module, as `clock_offset_ms` on each line. Run it against the shard's primary node, for the reason above:
