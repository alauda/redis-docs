...
Starting with Alauda Cache Service E1 v5.1.0, cross-datacenter replication offers two modes, selected per instance through `spec.activeRedis.mode` on the `Redis` resource.

| Mode | `spec.activeRedis.mode` | Topology | Membership resource | Redis versions |
|:-----|:------------------------|:---------|:--------------------|:---------------|
| Disaster Recovery | `peerof` (default) | Directed links. One upstream fans out to multiple downstreams (star). | `ActiveRedisConnection` | `6.0`, `7.2` |
| Active-Active | `mesh` | Every member accepts writes and replicates with every other member, with membership maintained by a gossip mesh. | `ActiveRedisMesh` | `7.2` only |

**Disaster Recovery** is the hot-standby topology: one instance takes the writes and the others follow it, ready to be promoted if a datacenter is lost. It is documented in [Set Up Disaster Recovery Replication](./20-setup.mdx).

**Active-Active** lets every datacenter serve reads and writes locally, with conflicts resolved automatically. It is documented in [Set Up Active-Active Replication](./40-mesh.mdx).
...
To move an existing Redis `6.0` replication group onto the new module, follow [Upgrade a Disaster Recovery Group from Redis 6.0 to Redis 7.2](./50-upgrade-6.0-to-7.2.mdx).
</Directive>
