...
Starting with Alauda Cache Service for Redis OSS v5.1.0, cross-datacenter replication offers two modes, selected per instance through `spec.activeRedis.mode` on the `Redis` resource.
