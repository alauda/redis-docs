...
* Unique identifier: Each Redis disaster recovery instance, whether it is a source or a target, must be configured with a globally unique `service_id`. The disaster recovery system uses this ID to establish a mapping between the source and target.
* Value range: The valid range of `service_id` is `[0-15]`.
* Topology limitation: Due to the ID range, a single disaster recovery source can be associated with up to 15 disaster recovery target instances, supporting a "one-source-multiple-target" topology.
