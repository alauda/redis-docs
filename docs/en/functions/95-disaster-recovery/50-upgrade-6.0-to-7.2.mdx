...
title: Upgrade a Disaster Recovery Group from Redis 6.0 to Redis 7.2
---
...
The new module requires a peer-auth credential on every instance. On the in-place upgrade of an instance that already had cross-datacenter replication enabled, the platform provisions one automatically — see [Automatic peer-auth provisioning](#automatic-peer-auth-provisioning).
...
In every case the resolution is the same: create the credential yourself, exactly as described in [Step 1 of the Disaster Recovery setup](./20-setup.mdx#step-1-create-the-peer-auth-credential-in-each-datacenter), using the **same username and password value across every member**, and bind it:
...
* **Active-Active mode**, in which every datacenter accepts writes instead of following a single upstream. See [Set Up Active-Active Replication](./40-mesh.mdx). Note that `spec.activeRedis.mode` selects the wiring for the instance; moving an established Disaster Recovery group to Active-Active is a separate change and is not covered by this procedure.
