...
You can create a Redis instance to achieve faster read and write speeds and scalable services, meeting the diverse and high availability needs of your business.

In the current version, the following Redis versions are supported: `5.0`, `6.0`, `7.2`; other versions are not supported by default.

<Directive type="tip" title="Version selection suggestion">
...
<Directive type="warning" title="Architecture selection suggestion">
...
<Tabs>
  ...
  <Tab label="Web Console">
    ...
    4. Select an architecture type.

    5. Refer to the instructions below to complete the relevant configurations.
           ...
           <table border="">
             ...
             <tr>
               ...
               <td>You can choose between system or custom parameter templates. For custom parameter templates please refer to <a href="../../unified-om/para-template-mgt/index">Parameter Templates</a>.</td>
             </tr>

             <tr>
               <td rowspan="3"><b>Connection Configuration</b></td>
               <td><b>Set Password</b></td>
               <td>To ensure the security of the environment, it is recommended that you set a password in a timely manner. <ul><li>If no password is set, the instance can be accessed without a password.</li><li>Once set, please keep your password secure. To reduce the risk of password leakage, it is recommended to <a href="../functions/20-user">update the password</a> regularly.</li></ul></td>
             </tr>

             <tr>
               <td><b>External Access to Cluster</b></td>
               <td><b>Sentinel Mode</b>: Once the instance is created, this configuration can no longer be modified. <br /><b>Cluster Mode</b>: Provides two external access modes: cluster mode and standalone mode. Cluster mode is recommended for its superior performance and stability.</td>
             </tr>

             <tr>
               <td><b>Specify Host Port</b></td>
               <td>When enabling specified host ports for external access, you can designate service port numbers. <br /><b>Sentinel Mode</b>: When updating the instance, ports cannot be swapped; if you need to swap ports, update to another unoccupied port first, then re-specify. <br /><b>Cluster Mode</b>: When updating the instance, if you only need to replace one port, you can re-enter or modify the corresponding node port in YAML.</td>
             </tr>

             <tr>
               <td rowspan="3"><b>Scheduling Configuration</b></td>
               <td><b>Anti-Affinity Policy</b></td>
               <td>To meet the demands for high reliability and high availability under different business scenarios, the platform provides an anti-affinity policy for <b>Cluster Mode</b>, where <b>intra-shard master-slave anti-affinity</b> is the default applied anti-affinity policy. <br /><b>Note</b>: Once the instance is created, this configuration can no longer be modified.</td>
             </tr>
